...
You can optionally pass `--debug` alongside `--obfuscate` to preserve the names of the `log`, `warn`, `error` and `breakpoint()` functions, making it easier to debug a garbled build. Also, if there are specific variables or functions you do not want garbled, simply prefix their names with `__` (this prefix will be stripped in the final output, leaving your original name intact).
